...
if( isset( $_POST['register'] ) ) {
    ...
    $password = esc_str( $_POST['password'] );
    $retype_password = esc_str( $_POST['re-type_password'] );

    if($password != $retype_password) {
        set_site_notice( 'Password does not match.', 'error' );
    }
    ...
    if( $check_user_result->num_rows <= 0 && $check_email_result->num_rows <= 0 && $password === $retype_password) {
        ...
        $_SESSION['uid'] = $reference;
                if( isset( $_POST['remember'] ) ) {
                    setcookie( 'uid', $reference, time() +2592000, '/' );
                }
                update_userinfo( 'status', 'online', $reference );
                redirect( 'admin/' );    
        $insert_user->close();
                    ...
                    <div class="login-content">
                        ...
                        <div class="login-form">
                            <form action="" method="post">
                                ...
                                <div class="form-group">
                                    <label>Password</label>
                                    <input class="au-input au-input--full" type="password" name="password" minlength="5" placeholder="Password">
                                </div>
                                <div class="form-group">
                                    <label>Re-type Password</label>
                                    <input class="au-input au-input--full" type="password" name="re-type_password" minlength="5" placeholder="Re-type Password">
                                </div>
                                <?php endif; ?>
                                ...
                                <?php endif; ?> -->
                                <?php if( empty( $action ) ) : ?>
                                <button class="au-btn au-btn--block au-btn--green m-b-20" type="submit" style="margin-top: 10%">sign in</button>
                                <input type="hidden" value="1" name="login">
                                <?php elseif( $action === 'register' ) : ?>
                                <button class="au-btn au-btn--block au-btn--green m-b-20" type="submit" style="margin-top: 10%">register</button>
                                <input type="hidden" value="1" name="register">
                                <?php elseif( $action === 'forgotpassword' ) : ?>
                                <button class="au-btn au-btn--block au-btn--green m-b-20" type="submit" style="margin-top: 10%">request login link</button>
