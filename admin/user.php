...
if( isset( $_POST['update-usermeta'] ) ) {
    ...
    set_site_notice( 'Personal information updated successfully.', 'success' );
}
        ...
        <div class="col-sm-12">
            <div class="card">
                ...
                <div class="card-footer">
                    <?php if( $action === 'add' ) : ?>
                        <button type="submit" style="float: right" class="btn btn-primary btn-md">Add User</button>
                        <input type="hidden" name="add-user" value="1">
                        <?php elseif( $action === 'edit' || empty( $action ) ) : ?>
                            <button type="submit" style="float: right" class="btn btn-primary btn-md">Save Changes</button>
                            ...
                            <div class="form-group">
                                <div class="form-row">
                                    <div class="col">
                                        ...
                                        <input type="text" value="<?php echo $current_usermetas['billing-firstname']; ?>" class="form-control" id="billing-firstname" name="billing-firstname" required>
                                    </div>
                                    <div class="col">
                                        <label for="billing-lastname">Last Name</label>
                                        <input type="text" value="<?php echo $current_usermetas['billing-lastname']; ?>" class="form-control" id="billing-lastname" name="billing-lastname" required>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="billing-telephone">Telephone or Mobile Number</label>
                                <input type="number" value="<?php echo $current_usermetas['billing-telephone']; ?>" name="billing-telephone" class="form-control" id="billing-telephone" pattern="[0-9]{11}" title="PLEASE PUT 11-DIGIT NUMBER WITH COUNTRY CODE" onKeyDown="if(this.value.length==11 && event.keyCode!=8) return false;" required>
                            </div>
                            <div class="form-group">
                                <label for="billing-address">House no., Purok, Sitio, Barangay</label>
                                <input type="text" value="<?php echo $current_usermetas['billing-address']; ?>" name="billing-address" class="form-control" id="billing-address" required>
                            </div>
                            </div>
                            <div class="card-footer">
                                <button type="submit" style="float: right" class="btn btn-primary btn-md">Save Changes</button>
