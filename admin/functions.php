...
function ajax_search_products() {
    ...
    if( ! empty( $search ) ) {
        $select_stmt = $mysqli->prepare( "SELECT * FROM products WHERE name LIKE ? ORDER BY date_added DESC" );
    ...
    if( $select_result->num_rows > 0 ) {
        while( $row = $select_result->fetch_assoc() ) {
            ...
            $html .= '<td>';
            $html .= '<a href="' . site_url( '/product.php?p=' . $row['slug'] ) . '" class="title">' . $row['item_code'] . '</a>';
            $html .= '</td>';
            $html .= '<td><a href="' . site_url( '/product.php?p=' . $row['slug'] ) . '" class="title">' . $row['name'] . '</a></td>';
            $html .= '<td> &#8369;';
            if( (int) $row['sale_price'] > 0 ) {
            $html .= '<span class="not">' . number_format($row['price'], 2) . '</span>';            
            }else {
            $html .= '<span>' . number_format($row['price'], 2) . '</span>';
            }
            if( (int) $row['sale_price'] > 0 ) {
            $html .=  '<span>' . number_format($row['sale_price'], 2) . '</span>';
            } 
            $html .= '</td>';
...
function ajax_search_orders() {
    ...
    if( $select_result->num_rows > 0 ) {
        while( $row = $select_result->fetch_assoc() ) {
            ...
            $html .= '<td>';
            if( $row['user_ID'] ) {
            $html .= '<a href="profile.php?id=' . $row['user_ID'] . '" class="title">' . get_userby( 'username', $row['user_ID'] ) . '</a>';
            };
            $html .= '</td>'; 
...
function ajax_search_users() {
    ...
    if( $select_result->num_rows > 0 ) {
        while( $row = $select_result->fetch_assoc() ) {
            ...
            $html .= '<td><a href="profile.php?id=' . $uid . '" class="title">' . $row['username'] . '</a></td>';
...
function ajax_pos_add_item() {
    global $mysqli;
    $html = '<tr>';
    $html .= '<td width="50"><a href="#" class="item-remove title-low"><i class="fa fa-times-circle"></i></a></td>';
