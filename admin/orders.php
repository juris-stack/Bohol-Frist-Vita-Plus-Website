                        ...
                        while( $row = $get_brand_result->fetch_assoc() ) : 
                            ...
                            $order_stmt->close();?>
                             <?php
                                ...
                                $oddate=date_format($date1,"d");
                                $diff = $today-$oddate;
                                $d=strtotime("+2 Days");
                                $picdate=date("Y-m-d", $d);
                                
                                
                                if($diff==1&&$status=='pending'&&$notif==0){
                                $stmt_update = $mysqli->prepare( "UPDATE orders SET notif=$diff WHERE ID= $order_id");
                                    $stmt_update->execute();
                                    $stmt_update->close();    
                                $result = itexmo("$phone","Good Day ".$fname."! Your Reservation with Order ID ".$order_id." is ready for pick up. Please claim your resevation today or it will be cancelled. Please contact 09338205013 for final confirmation. Upon receiving this SMS, you only have 3 hours to confirm.\n\n-First Vita Plus","ST-TANNY420621_B7B7J");
                                if ($result == ""){
                                echo "";  
                                }else if ($result == 0){
                                echo "";
                                ...
                                else if($diff==2&&$status=='pending'&&$notif==1){
                                    $stmt_update = $mysqli->prepare( "UPDATE orders SET notif=$diff WHERE ID= $order_id");
                                    $stmt_update->execute();
                                    $stmt_update->close(); 
                                $result = itexmo("$get_number","Good Day Admin! Reservation with Order ID ".$order_id." is unclaimed. You may now cancel the Reservation.\n\n-First Vita Plus","ST-TANNY420621_B7B7J");
                                if ($result == ""){
                                echo "";  
                                }else if ($result == 0){
                                echo "";
                                ...
                                ?>
                            <tr class="tr-shadow">
                                <td><a href="<?php echo site_url( '/admin/order.php?action=edit&id=' . $order_id ); ?>" class="title"><?php echo $order_id; ?></a></td>
                                ...
                                <td>
                                    <div class="table-data-feature">
                                        ...
                                        <!-- <a href="<?php echo site_url( '/admin/order.php?action=delete&id=' . $order_id ); ?>" class="item js-confirm" data-toggle="tooltip" data-placement="top" title="Delete"> -->
