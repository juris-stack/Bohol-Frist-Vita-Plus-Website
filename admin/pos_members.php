...
if( isset( $_POST['pos-products'] ) ) {
    ...
    if($received < $total){
        ...
        $insert_order = $mysqli->prepare( "INSERT INTO orders (type, amount, products, status, customer_type) VALUES (?, ?, ?, ?, ?)" );
        $insert_order->bind_param( 'sssss', $type, $total, $products_serialize, $status, $disc );
...
$site_title = 'Members (25% Discount)';
            ...
            <table class="table table-data2">
                    ...
                    if( is_array( $products ) && count( $products ) > 0 ) : 
                        foreach( $products as $id => $qty ) :
                            ...
                            <tr data-price="<?php echo $price; ?>">
                                <td width="50"><a href="#" class="item-remove title-low"><i class="fa fa-times-circle"></i></a></td>
                                <td>
                ...
                if( is_array( $products ) && count( $products ) > 0 ) {
                    ...
                    <tr style="font-size: 20px;">
                        ...
                        <td>&#8369; <span id="pos-sub-total" data-total="<?php echo $total; ?>"><?php echo number_format($total,2); ?></span></td>
                    </tr>
                    <tr style="font-size: 20px;">
                        <td colspan="4" class="text-right">Discount</td>
                        <td>&#8369; <span id="pos-sub-total" data-total="<?php echo $total; ?>">25%</span></td>
                    </tr>
                    <tr style="font-size: 20px;">
                        <td colspan="4" class="text-right">Total Sales</td>
                        <td>&#8369; <span id="pos-total" data-total="<?php echo $total; ?>"><?php echo number_format($total,2); ?></span></td>
                    </tr>
                    <tr style="font-size: 20px;">
                        <td colspan="4" class="text-right">Amount Received</td>
                        <td>&#8369; <input id="pos-received" type="number" class="form-control" style="width: 100px; display: inline;" value="<?php echo $received; ?>"></td>
                    </tr>
                    <tr style="font-size: 20px;">
...
?>
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="copyright">
                                        <p>Copyright © <?php echo date( 'Y' ); ?> <?php echo get_siteinfo( 'site-name' ); ?>. All rights reserved.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <script src="../assets/js/admin-members.js?ver=<?php echo filemtime( '../assets/js/admin-members.js' ); ?>"></script>
    </body>
</html>