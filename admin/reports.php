...
<div class="row">
    <div class="col-md-12">
        <div class="overview-wrap">
            <h2 class="title-1">Sales Reports</h2>
        </div>
    </div>
</div>
    ...
    <div class="card-body">
        ...
        <div class="tab-content pl-3 p-1" id="myTabContent">
            <div class="tab-pane fade show active" id="today" role="tabpanel" aria-labelledby="today-tab">
                ...
                if( $role == '4' ) {
                    $today_earnings_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE status != 'pending' AND DATE(date_added) = CURDATE() ORDER BY date_added DESC" );
                }else if( $role == '3' ){
                    $today_earnings_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE status != 'pending' AND DATE(date_added) = CURDATE() AND managed_by = '$managed_by' ORDER BY date_added DESC" );
                }
                        ...
                        <table class="table table-data2">
                            ...
                            <tbody>
                                <?php foreach( $todays as $today ) : ?>
                                    <tr class="tr-shadow">
                                        <td style="font-size: 12px;"><a href="<?php echo site_url( '/admin/order.php?action=edit&id=' . $today['ID'] ); ?>"><?php echo $today['ID']; ?></a></td>
                                        <td style="font-size: 12px;"><?php echo $today['type']; ?></td>
                                        ...
                                        <td style="font-size: 12px;"><?php if( $today['user_ID'] ) {
                                            echo '<a href="profile.php?id=' . $today['user_ID'] . '">' . get_userby( 'username', $today['user_ID'] ) . '</a>';
                                        } ?></td>
                                        <td style="font-size: 12px;"><?php echo $today['managed_by']; ?></td>
                                        <td style="font-size: 12px;">&#8369;<?php echo number_format($today['amount'], 2); ?></td>
                                        <td style="font-size: 12px;"><?php echo $today['date_added']; ?></td>
                                    </tr>
                                    <tr class="spacer"></tr>
                                <?php 
                                endforeach;
            ...
            <div class="tab-pane fade" id="yesterday" role="tabpanel" aria-labelledby="yesterday-tab">
                ...
                $role = get_currentuser( 'role' );
                if( $role == '4' ) {
                    $yesterday_earnings_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE status != 'pending' AND DATE(date_added) = CURDATE() - INTERVAL 1 DAY ORDER BY date_added DESC" );
                }else if( $role == '3' ){
                    $yesterday_earnings_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE status != 'pending' AND DATE(date_added) = CURDATE() - INTERVAL 1 DAY AND managed_by = '$managed_by' ORDER BY date_added DESC" );
                }
            ...
            <div class="tab-pane fade" id="week" role="tabpanel" aria-labelledby="week-tab">
                ...
                $role = get_currentuser( 'role' );
                if( $role == '4' ) {
                    $week_earnings_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE status != 'pending' AND YEARWEEK(date_added) = YEARWEEK(CURDATE()) ORDER BY date_added DESC" );
                }else if( $role == '3' ){
                    $week_earnings_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE status != 'pending' AND YEARWEEK(date_added) = YEARWEEK(CURDATE()) AND managed_by = '$managed_by' ORDER BY date_added DESC" );
                }
            ...
            <div class="tab-pane fade" id="month" role="tabpanel" aria-labelledby="month-tab">
                ...
                $role = get_currentuser( 'role' );
                if( $role == '4' ) {
                    $month_earnings_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE status != 'pending' AND MONTH(date_added) = MONTH(CURDATE()) ORDER BY date_added DESC" );
                }else if( $role == '3' ){
                    $month_earnings_stmt = $mysqli->prepare( "SELECT * FROM orders WHERE status != 'pending' AND MONTH(date_added) = MONTH(CURDATE()) AND managed_by = '$managed_by' ORDER BY date_added DESC" );
                }
